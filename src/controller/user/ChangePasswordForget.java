...
/**
 * Servlet implementation class ChangePasswordForget
 */
@WebServlet("/ChangePasswordForget")
...
//		System.out.println(passwordConfirm + " "+ password);
		...
		if (!verificationCode.equals(verificationCodeConfirm)) {
			out.print("<script language='javascript'>alert('验证码不正确，请输入正确验证码!');window.location.href='forgetpassword.jsp';</script>");
		} 
		else if (!password.equals(passwordConfirm)) {
			out.print(
					"<script language='javascript'>alert('两次输入的密码不一致，请输入相同的密码!');window.location.href='forgetpassword.jsp';</script>");
		}	
		else {
